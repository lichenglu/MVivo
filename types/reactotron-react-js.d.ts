declare module "reactotron-react-js" {
	...
	export interface ReactotronUseReactAsync {
		ignore: string[];
	}

	export interface ReactotronUseReactNetworking {
		ignoreContentTypes?: RegExp;
		ignoreUrls?: RegExp;
	}

	export interface ReactotronUseReactEditor {
		url?: string;
	}

	export interface ReactotronUseReactErrors {
		veto: (stackFrame: ReactotronStackFrame) => boolean;
	}

	export interface ReactotronUseReactOptions {
		asyncStorage?: false | ReactotronUseReactAsync;
		networking?: false | ReactotronUseReactNetworking;
		editor?: false | ReactotronUseReactEditor;
		errors?: false | ReactotronUseReactErrors;
		overlay?: false;
	}
	...
	export interface Reactotron {
		 ...
		 */
		configure(options?: ReactotronConfigureOptions): Reactotron | undefined;

		/**
		 * Connect to the Reactotron app.
		 */
		connect(): Reactotron | undefined;

		/**
		 * Registers a plugin.
		 */
		use(
			middleware: (tron: Reactotron) => ReactotronPlugin
